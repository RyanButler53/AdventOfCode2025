...
template <typename T, size_t K>
double KDTree<T,K>::dist(DistanceFunction fn, const key_t& p1, const key_t& p2) {
    T sum = 0;
    switch (fn)
    {
...
typename KDTree<T,K>::key_t KDTree<T,K>::nearestNeighbor(const key_t& query, DistanceFunction fn) const {
    ...
    std::vector<key_t> nn = nearestNeighbor(query, 2, fn);
    return nn[0];
}
...
std::vector<typename KDTree<T,K>::key_t> KDTree<T,K>::nearestNeighbor(const key_t& query, size_t k, DistanceFunction fn) const {
    ...
    // Don't return the query point
    if (neighbors.back() == query){
        neighbors.pop_back();
    }
